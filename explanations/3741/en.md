...
**Brute force vs. optimized strategy**
  ...
  * **Brute Force:** Try all possible triplets $(i, j, k)$ where $nums[i] == nums[j] == nums[k]$. This would be $O(n^3)$.
  * **Optimized (Grouping + Consecutive Triplets):** Group indices by value using a hash map. For each value with at least 3 occurrences, check consecutive triplets in the sorted list of indices. Since we want to minimize $(r - p)$, checking consecutive triplets is optimal because any non-consecutive triplet would have a larger span.
