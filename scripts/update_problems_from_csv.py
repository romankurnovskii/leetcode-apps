...
def map_topics_to_category(topics_str):
    ...
    if any('database' in t for t in topics_lower):
        ...
        return 'Tree'
    elif any('graph' in t or 'dfs' in t or 'depth-first' in t or 'bfs' in t or 
             'breadth-first' in t or 'union find' in t for t in topics_lower):
        return 'Graph Traversal'
    elif any('dynamic programming' in t or 'dp' in t or 'memoization' in t for t in topics_lower):
        ...
        return 'Greedy'
    elif any('math' in t or 'geometry' in t or 'number theory' in t or 
             'combinatorics' in t for t in topics_lower):
...
def save_json(data):
    ...
    try:
        ...
        sorted_items = sorted(data.items(), key=lambda item: int(item[0]) if item[0].isdigit() else float('inf'))
